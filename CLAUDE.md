...
| How to run it in a container | [docs/deployment.md](docs/deployment.md) |
...
CI (`.github/workflows/tests.yml`) runs four jobs on every pull request and on pushes to `main`: **Format and lint** (`cargo fmt --check` and `cargo clippy --all-targets -- -D warnings`), **Unit tests** (`cargo test`), **Functional tests** (`pytest`) and **Container image** (`docker build`, then a started container serving a tip and its own healthcheck agreeing). All four must pass, and the lint job is separate so a red build says at a glance which kind of problem it is.
...
- `health.rs`: the `health` subcommand, and the container's healthcheck. Up is not the same as working — a node whose miner has wedged answers `/status` perfectly well — so it asks whether the **tip has moved**, which needs a memory: the height last seen and when. The marker lives in the container rather than the data directory, because a restarted container should start the clock again rather than inherit a verdict about the node it replaced, and nothing but the node writes to the node's directory. A marker it cannot write makes every check look like a first one, which reports healthy: a healthcheck that failed the container over its own scratch file would be worse than one that is blind.
