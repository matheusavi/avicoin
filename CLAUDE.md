...
- `data_dir` defaults to `.avicoin` under the home directory, and is where one node keeps its chain, its UTXO set and its key. **One node per directory.** `DataDir::open` creates it, stamps it with the network that built it, and refuses one another network built ([ADR-0013](docs/adr/0013-persistence.md)) — before the listener binds, so a wrong path costs no port. An empty `data_dir` is a startup error rather than an absent value, the same rule `host_address` follows.
...
- `data_dir.rs`: `DataDir::open` creates the per-node directory, stamps it with the network's name and genesis hash, and refuses a directory another network built — the ADR-0007 separation applied to disk. `main` opens it **before binding the listener**. Nothing is stored in it yet.
