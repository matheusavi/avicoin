...
**Identity is the `version` nonce, never an address.** `Node::identify` runs when a `version` arrives: our own nonce means we dialled ourselves and the connection goes; a nonce already in the table means this is a second connection to one peer, and exactly one survives. The survivor is chosen by `survivor_is_dialled` — *the connection dialled by the larger nonce* — because both ends run the same rule over the same pair of nonces under **opposite** origins, and any rule phrased from one node's point of view ("keep the one we dialled") makes each drop what the other kept. There is no address dedup left in `PeerTable`; an accepted connection shows an ephemeral source port, which is why it never worked.
