...
**Relay**: accepting a transaction into the mempool offers its txid to every Ready peer *except* the one it came from, and a mined block is offered to all of them. A block whose parent is unknown is held and its parent asked for from the peer that sent it, so a chain walks back one block at a time until it connects — headers-first sync replaces that walk in #91. An `inv` for a txid we do not hold produces a `getdata` **to that peer only** — a broadcast there would have every peer fetching what one of them offered. An `inv` for something we hold produces nothing, which is what stops two nodes trading the same payment forever. A `tx` goes through the same `Mempool::accept` a locally built one does: relay is not a way around validation.
