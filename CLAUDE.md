...
**Relay** (M3): accepting a transaction into the mempool offers its txid to every Ready peer *except* the one it came from. An `inv` for a txid we do not hold produces a `getdata` **to that peer only** — a broadcast there would have every peer fetching what one of them offered. An `inv` for something we hold produces nothing, which is what stops two nodes trading the same payment forever. A `tx` goes through the same `Mempool::accept` a locally built one does: relay is not a way around validation.
