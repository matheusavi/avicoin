...
Nothing calls `println!` outside `Node::record`, which writes to stdout *and* a bounded `RingBuffer` on the shared node — M6's HTTP API reads the buffer. `node::record(&node, ...)` is the convenience that takes the lock for you, so **never call it while already holding the node lock**: std's `Mutex` is not reentrant, and the borrow checker does not stop you (a held guard and a helper that locks are both immutable borrows of the same `Arc`).
