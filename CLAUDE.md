...
- `api.rs`: one thread and a fixed pool of `WORKERS` behind it — a thread per connection is a thread per stranger. `route` takes the node lock, copies what it needs and **gives it back before returning**; everything after it writes to a socket a stranger controls the read end of, which is the same rule `record` follows for stdout. A handler's panic becomes a 500 and the worker takes the next request. `main` binds the port so a taken one fails the process. Shapes are documented in [docs/api.md](docs/api.md) because M7's scenarios read them.
