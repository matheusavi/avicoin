...
| `config.rs` | Defaults → `config.toml` → CLI, validated into `SocketAddr` | Built |
...
| Config / CLI | **Keep** `toml` + `serde`; **add** `clap` (it was documented as present but was not — added on the config branch). |
